...
#define MEET_IN_THE_MIDDLE 0
#define LEGACY_SNIPPER 0
...
OpPtT FoundLimits::snipTrack(CurveRef curveRef, const OpPtT& common, float step) {
	OpSegment* seg = cc->seg;
	OpSegment* opp = cc->opp;
	float last = step;
	float maxDistance = cc->maxDist * cc->context->thresholdLength;
	OpPtT snip;
	if (CurveRef::opp == curveRef)
		std::swap(seg, opp);
	float lastSnipT = common.t;
	for (;;) {
		snip.t = std::min(1.f, std::max(0.f, common.t + step));
		if (snip.t == lastSnipT)
			break;
		lastSnipT = snip.t;
		snip.pt = seg->c.ptAtT(snip.t);
		OpPtT oppSnip = seg->distance(snip, opp);
		if (OpMath::IsNaN(oppSnip.t))
			break;
		float distance = (oppSnip.pt - snip.pt).length();
...
bool FoundLimits::addSnip(OpIntersection* sect, const OpPtT& sPtT, const OpPtT& oPtT, float scale) {
#if LEGACY_SNIPPER  // !!! approach that does not use distance to opposite curve (converges slowly)
	SnipPtTs snipLo { sPtT, oPtT };
	SnipPtTs snipHi { sPtT, oPtT };
	if (!cutPair(snipLo, snipHi))
		return false;
	return addSnipCommon(snipLo, snipHi);
#else
	auto getTStep = [this, scale](OpSegment* s) {
		const OpContext& context = *cc->context;
		PathOpsV0Lib::CurveConst cutFun = context.callbacks[s->c.c.type].maxCutFuncPtr;
		float tStep = cutFun ? (*cutFun)(s->c.c) : 16.f;
		return OpEpsilon * tStep * scale;
	};
	float segStep = getTStep(cc->seg);
	SnipPtTs snip { nullptr, sPtT, oPtT };
	snip.segCut.lo = 0 == sPtT.t ? sPtT : snipTrack(CurveRef::edge, sPtT, -segStep);
	snip.segCut.hi = 1 == sPtT.t ? sPtT : snipTrack(CurveRef::edge, sPtT, segStep);
	float oppStep = getTStep(cc->opp);
	snip.oppCut.lo = 0 == oPtT.t ? oPtT : snipTrack(CurveRef::opp, oPtT, -oppStep);
	snip.oppCut.hi = 1 == oPtT.t ? oPtT : snipTrack(CurveRef::opp, oPtT, oppStep);
#if 0
	// check if both snips are on same side of other snips
	LinePts segSnipPts { snip.segCut.lo.pt, snip.segCut.hi.pt };
	OpRoots segSnipRoots = cc->opp->c.lineIntersect(segSnipPts);
	for (float root : segSnipRoots.roots) {
		OpPtT segSectPt = cc->opp->c.ptTAtT(root);
		float segClosest = cc->seg->c.matchClosest(segSectPt.pt);
		if (!OpMath::Between(snip.segCut.lo.t, segClosest, snip.segCut.hi.t))
			OpNop();
		// limit should be adjusted to this point
		// add snip should be recomputed from this new point
	}
#endif
	return addSnipCommon(sect, snip, snip);
#endif
}

bool FoundLimits::addSnipCommon(OpIntersection* sect, SnipPtTs& snipLo, SnipPtTs& snipHi) {
	auto snipPush = [this, sect, snipLo, snipHi]() {
		snips.push_back({ sect, snipLo.segPtT, snipLo.oppPtT, { snipLo.segCut.lo, snipHi.segCut.hi },
	...
	for (size_t index = 0; index < snips.size(); ) {
		...
		if (!copySnip)
			...
			snips[index] = { sect, snipLo.segPtT, snipLo.oppPtT, { snipLo.segCut.lo, snipHi.segCut.hi },
					{ snipLo.oppCut.lo, snipHi.oppCut.hi } };
			++index;
			copySnip = false;
		}
...
bool FoundLimits::addSnipRange(size_t start) {
    ...
    for (size_t index = start; index < i.size(); ++index) {
        for (;;) {
            addSnip(nullptr, i[index].segPtT, i[index].oppPtT, scale);
        ...
        // limit and increase snip radius until they do; use larger edges to proceed
            if (snips.empty())
                break;
            SnipPtTs& snip = snips.back();
            OpRect edgeSnip { snip.segCut.lo.pt, snip.segCut.hi.pt };
            OpRect oppSnip { snip.oppCut.lo.pt, snip.oppCut.hi.pt };
            if (edgeSnip.intersects(oppSnip))
                break;
            snips.pop_back();
            scale += 1;
        }
...
bool FoundLimits::cutPair(SnipPtTs& snipLo, SnipPtTs& snipHi) const {
	...
	if (!OpMath::IsFinite(segLen) || !OpMath::IsFinite(oppLen))
		return false;
...
bool FoundLimits::setEnds(std::vector<OpIntersection*>& matchingSects) {
	cc->reversed = false;
...
void CcCurves::checkSigns(CurveRef curveRef) {
	if (c.empty())
		return;
	for (OpEdge* edge : c) {
		if (OpMath::IsNaN(edge->startDist.opp.t) || OpMath::IsNaN(edge->endDist.opp.t))
			continue;
		const EdgeRun* last = &runs[0];
		for (size_t index = 1; index < runs.size(); ++index) {
			const EdgeRun* run = &runs[index];
//			OP_ASSERT(run->oppDist);  // !!! if dist is zero, more code needs to be written
			if (!OpMath::IsNaN(last->oppDist)) {
				if (!(last->oppDist * run->oppDist > 0))  // check for NaN
					checkOneSign(last, run, curveRef);
			}
			last = run;
		}
	}
...
void CcCurves::checkOneSign(const EdgeRun* start, const EdgeRun* end, CurveRef curveRef) {
	// use distance to weight middle of binary search
	float startT = start->edgePtT.t;
	float endT = end->edgePtT.t;
	...
	if (edgeLen > scaledMax || oppLen > scaledMax)
		return;
	...
	if (opp->sects.contains(run.oppPtT, seg))
		return;
#if 0  // defer results by adding to limits, so that coincident / unsectable runs can be found
...
OpCurveCurve::OpCurveCurve(OpSegment* s, OpSegment* o, std::vector<OpIntersection*>& matchingSects, 
    ...
    overflowFail = !limits.setEnds(matchingSects);
}
...
bool OpCurveCurve::addUnsectable(FoundLimit& limit, FoundLimit& limitEnd) {
...
#if MEET_IN_THE_MIDDLE  // !!! this causes downstream errors; the meet-mid pt may make the curve non-monotonic
	...
	IsCoin isCoin = eStart.pt == oStart.pt && eEnd.pt == oEnd.pt ? IsCoin::yes : IsCoin::no;
...
OpEdge* OpCurveCurve::boundedEdge(OpSegment* segm, const OpPointBounds& sectBounds,
	...
	auto saveRoots = [&c, saveBest, safetyLimit, this](Axis axis, float inside, float boundary) {
		...
		for (float root : roots.roots) {
            ...
            OP_ASSERT(safetyCount > 0 || OpDebugExpect::fail == context->debugExpect);
			saveBest(ptT, axis);
		}
	};
...
bool OpCurveCurve::checkSplit(float loT, float hiT, CurveRef which, OpPtT& checkPtT) {
	OP_ASSERT(loT <= checkPtT.t && checkPtT.t <= hiT);
	std::vector<OpEdge*>& oCurves = CurveRef::edge == which ? oppCurves.c : edgeCurves.c;
	...
	do {
		...
		// below expects edges in opp curves to be sorted
		std::sort(oCurves.begin(), oCurves.end(), [](const OpEdge* a, const OpEdge* b) {
				return a->startT < b->startT; } );
		for (OpEdge* oCurve : oCurves) {
			OpPtT delHi = oCurve->startPtT();
	#if OP_DEBUG_DUMP
			if (delLo.t < delHi.t)
				context->dumpFile("delLoHiErr");
	#endif
...
// Some checks require multiple depth passes. Runs are cleared by distance check, then found on 
//  next pass by snipping, the rechecked by distance on third pass. So, max splits check needs to
//  wait multiple successive passes before giving up.
SectFound OpCurveCurve::divideAndConquer() {
	...
	for (depth = 1; depth < maxDeep; ++depth) {
		...
		if (edgeOverlaps >= maxSplits || oppOverlaps >= maxSplits) {
			...
			edgeCurves.checkSigns(CurveRef::edge);
			oppCurves.checkSigns(CurveRef::opp);
			if (INT_MAX == maxDepth)
				maxDepth = depth + 1;  // allow dist flip check to complete before giving up
		}
		if (checkSect())
			snipEm = true;
		else if (!snipEm) {
		...
		if (!snipEm) {
			...
			// check if added limits suggest that range of edge and opp are coincident
			// for now, if limit count exceeds hull count, look for unsectable / coincidence
			int edgeHulls = parentEdge->curve.pointCount() - 1;  // # of control lines
			int oppHulls = parentOpp->curve.pointCount() - 1;
			if (limits.size() - limitCount >= ((int) edgeCurves.c.size() * edgeHulls 
					+ (int) oppCurves.c.size() * oppHulls))
				return SectFound::add;
            OP_DEBUG_CODE(edgeCurves.debugAdd(eSplits));
            OP_DEBUG_CODE(oppCurves.debugAdd(oSplits));
...
void OpCurveCurve::findUnsectable() {
	...
	auto addSect = [this, &unsectLo, &unsectHi]() {
		...
		if (LimitFrom::yes == limit.fromFoundT)
			return;
#if MEET_IN_THE_MIDDLE  // !!! causes downstream errors since met-in-mid pt may not be monotonic
	...
	for (size_t index = 1; index < limits.size(); ++index) {
		...
		if (oppdist.length() <= edgeCurves.scaledMax)
			unsectHi = index;
		else {
			addSect();
			unsectLo = index;
...
bool OpCurveCurve::reduceDistFlipped() {
	...
	bool swap = false;
    auto addSplit = [this, &curves](OpSegment* segment, CcCurves& splits, 
			OpPtT& lower, OpPtT& upper) {
	...
	do {
		...
		auto keepRun = [addSplit, segment, &lower, &upper, &curves](CcCurves& splits) {
            ...
            addSplit(segment, splits, lower->edgePtT, upper->edgePtT);
		};
		...
		CcCurves& oSplits = swap ? edgeSplits : oppSplits;
        EdgeRun* prior = nullptr;  // order: prior, lower, upper (same as run)
		for (auto& run : curves->runs) {
            ...
            auto splitClose = [&segment, &oSegment, addSplit, &splits, &oSplits](
                ...
                if (temp.oppDist * lower->oppDist < 0
                    ...
                    addSplit(segment, splits, prior->edgePtT, tempPtT); 
                    addSplit(segment, splits, tempPtT, lower->edgePtT);
					// !!! as experiment: add opp since it may not have detectable run angles
					std::array<OpPtT, 3> oppPtTs = { prior->oppPtT, oppPtT, lower->oppPtT };
					std::sort(oppPtTs.begin(), oppPtTs.end(), [](const OpPtT& a, const OpPtT& b) {
							return a.t < b.t; });
					addSplit(oSegment, oSplits, oppPtTs[0], oppPtTs[1]);
					addSplit(oSegment, oSplits, oppPtTs[1], oppPtTs[2]);
               }
            };
			if (switchesSides && !nearlyZero && !byZero)
				keepRun(splits);
            else if (OpEdge* edge = curves->twoHulls(lower->edgePtT, run.edgePtT)) {
				if (!edge->centerless) {
					OpPtT midPtT = edge->center;
					addSplit(segment, splits, lower->edgePtT, midPtT);
					addSplit(segment, splits, midPtT, run.edgePtT);
				}
		...
		#if 0   // !!! This splits wrong run. Unsure that t range is a reliable indicator of
				//  which run to split. 
		...
		#else  // Try (as an experiment) ignoring t and splitting both.
				splitClose(prior, lower);
				splitClose(lower, &run);
		#endif
            }
...
bool OpCurveCurve::setHullSects(OpEdge& edge, OpEdge& oppEdge, CurveRef curveRef) {
	...
	// always use original segment to find points
			...
			if (!oppPtT.pt.isFinite())
				return false;
			alignAndRecord(edge, oppEdge, sectPtT, oppPtT, curveRef);
			return true;
		}
	}
    return false;
...
bool OpCurveCurve::addLineCurveIntersection(OpEdge& edge, OpEdge& oppEdge, CurveRef curveRef) {
    ...
    auto checkIntersection = [&edge, &oppEdge, segment, opposite, curveRef, this](OpRoots& oppRoots) {
        for (float oppT : oppRoots.roots) {
			...
			alignAndRecord(edge, oppEdge, edgePtT, oppPtT, curveRef);
            return true;
        }
        return false;
    };
...
void OpCurveCurve::alignAndRecord(OpEdge& edge, OpEdge& oppEdge, OpPtT& sectPtT, OpPtT& oppPtT,
		CurveRef curveRef) {
	...
	auto tryIt = [this](const LinePts& eLinePts, const LinePts& oLinePts, OpCurve& eCurve,
			OpPoint* eLinePt) {
		...
		if (!OpMath::IsFinite(eT))
			return false;
		*eLinePt = (1 - eT) * eLinePts.pts[0] + eT * eLinePts.pts[1];
		return true;
	};
	OpPtT eTry, oTry;
	if (tryIt(eLinePts, oLinePts, edge.curve, &eTry.pt) 
			&& tryIt(oLinePts, eLinePts, oppEdge.curve, &oTry.pt)) {
		OpSegment* s = seg;
		OpSegment* o = opp;
		if (CurveRef::opp == curveRef)
			std::swap(s, o);
		eTry.t = s->c.findValidT(0, 1, eTry.pt);
		eTry.pt = s->c.ptAtT(eTry.t);
		oTry.t = o->c.findValidT(0, 1, oTry.pt);
		oTry.pt = o->c.ptAtT(oTry.t);
		float tryLength = (eTry.pt - oTry.pt).lengthSquared();
		float diffLength = (sectPtT.pt - oppPtT.pt).lengthSquared();
		if (tryLength < diffLength) {
			sectPtT = eTry;
			oppPtT = oTry;
		}
	}	
